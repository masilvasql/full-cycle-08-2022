...
func (t *TransactionRepositoryDb) SaveTransaction(transction domain.Transaction, creditCard domain.CreditCard) error {
	...
	_, err = stmt.Exec(
		transction.ID,
		transction.CreditCardId,
		transction.Amount,
		transction.Status,
		transction.Description,
		transction.Store,
		transction.CreatedAt,
	)

	if err != nil {
		return err
	}
	if transction.Status == "approved" {
